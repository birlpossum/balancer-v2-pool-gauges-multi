...
const SUBGRAPH_URLS: Record<string, { gauges: string; v2pools: string }> = {
  "1": {
    gauges:
      "https://gateway.thegraph.com/api/[api-key]/deployments/id/QmNrMRgSeUUkQsvhE6ExBEPETZ6P2jiJL3SzXftNQcAEcW",
    v2pools:
      "https://gateway.thegraph.com/api/[api-key]/deployments/id/QmQ5TT2yYBZgoUxsat3bKmNe5Fr9LW9YAtDs8aeuc1BRhj",
  },
  "34443": {
    gauges:
  ...
  "1101": {
    gauges:
      "https://gateway.thegraph.com/api/[api-key]/deployments/id/QmbarCL8ofjqRJVAuSvKpd99m9xetBEGixoEQHJx31WEdB",
    v2pools:
      "https://gateway.thegraph.com/api/[api-key]/deployments/id/QmVNcphdNjTCaJ4ammPtFzRyD4gmyNBomKcVFZ98RADpS1",
  },
  "137": {
    gauges:
      "https://gateway.thegraph.com/api/[api-key]/deployments/id/QmewSgLJf9TZt8trr61dECJhEGGyHxKFWbNQ3AnNZAdYyU",
    v2pools:
      "https://gateway.thegraph.com/api/[api-key]/deployments/id/QmUqS6BAVQgvstEsVrxuwsu1DwQdfAdj3Q6gz2j3DbUYQ9",
  },
  "10": {
    gauges:
...
class TagService implements ITagService {
  returnTags = async (chainId: string, apiKey: string): Promise<ContractTag[]> => {
    ...
    if (!/^\d+$/.test(trimmedChainId)) {
      throw new Error(`Unsupported Chain ID: ${originalChainId}. Only 1 (Ethereum), 10 (Optimism), 100 (Gnosis), 137 (Polygon), 43114 (Avalanche), 8453 (Base), 1101 (Polygon zkEVM), 42161 (Arbitrum), 11155111 (Sepolia), and 34443 (Mode) are supported in this module.`);
    }
    const chainIdNum = Number(trimmedChainId);
    if (!Number.isInteger(chainIdNum) || (chainIdNum !== 1 && chainIdNum !== 10 && chainIdNum !== 100 && chainIdNum !== 137 && chainIdNum !== 43114 && chainIdNum !== 8453 && chainIdNum !== 1101 && chainIdNum !== 42161 && chainIdNum !== 11155111 && chainIdNum !== 34443)) {
      throw new Error(`Unsupported Chain ID: ${originalChainId}. Only 1 (Ethereum), 10 (Optimism), 100 (Gnosis), 137 (Polygon), 43114 (Avalanche), 8453 (Base), 1101 (Polygon zkEVM), 42161 (Arbitrum), 11155111 (Sepolia), and 34443 (Mode) are supported in this module.`);
    }
